...
struct AddDataView: View {
    ...
    @ObservedObject var health: HealthManager // Usare l'istanza esistente
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 20) {
                ...
                VStack {
                    Text("\(Int(health.timeInBed)) secondi")
                        .font(.largeTitle)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                    
                    Stepper(value: Binding(
                        get: { Int(health.timeInBed) },
                        set: { health.timeInBed = Double($0) }
                    ), in: 0...86400, step: 60) {
                        Text("Regola il tempo").foregroundColor(.gray)
                    }
                    .padding()
                    .background(RoundedRectangle(cornerRadius: 12).fill(Color.gray.opacity(0.2)))
                    .onChange(of: health.timeInBed) { oldValue, newValue in
                        let now = Date()
                        UserDefaults.standard.set(newValue, forKey: "timeInBed")
                ...
                Button(action: {
                    ...
                    Task {
                        let success = await health.saveSleepData(startDate: startDate, endDate: sleepDate)
                        if success {
