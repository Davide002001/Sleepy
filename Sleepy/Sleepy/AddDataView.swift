        ...
        NavigationStack {
            VStack(spacing: 20) {
                ...
                Button(action: {
                    ...
                    Task {
                        // Verifica se esiste già un periodo di sonno sovrapposto
                        await health.deleteExistingSleepData(startDate: selectedStartDate, endDate: selectedEndDate)

                        let success = await health.saveSleepData(startDate: selectedStartDate, endDate: selectedEndDate, sleepType: .inBed)
                        if success {
                            print("✅ Dati salvati su HealthKit")
                            apiResponse += "\n✅ Dati salvati su HealthKit"
                            health.fetchSleepdata()  // Ricarica i dati
