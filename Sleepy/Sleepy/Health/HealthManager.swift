...
class HealthManager: ObservableObject{
    ...
    func fetchSleepdata() {
        ...
        let query = HKSampleQuery(sampleType: sleepType, predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: nil) { _, results, error in
            ...
            // ✅ Rimuove eventuali duplicati tra i dati recuperati
            let uniqueSamples = Array(Set(results))

            // ✅ Calcola la durata totale del sonno sommando tutti i campioni unici
            let totalSleep = uniqueSamples.reduce(0) { $0 + $1.endDate.timeIntervalSince($1.startDate) }

            // ✅ Aggiorna l'interfaccia utente con il valore calcolato
            DispatchQueue.main.async {
                self.timeInBed = totalSleep
                print("🕒 Tempo totale di sonno: \(totalSleep) secondi (\(totalSleep / 60) minuti)")
            }
        }
